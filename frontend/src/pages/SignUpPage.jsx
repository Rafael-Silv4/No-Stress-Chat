...
const SignUpPage = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.email.trim()) return toast.error("Email obrigatório");
    if (!/\S+@\S+\.\S+/.test(formData.email)) return toast.error("formato de email inválido");
    if (!formData.password) return toast.error("Senha obrigatória");
    if (formData.password.length < 6) return toast.error("Senha precisa ter no minimo 6 caracteres");
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      <AuthImagePattern
        ...
        subtitle="Conecte-se com amigos, compartilhe momentos e mantenha-se em contato com seus entes queridos."
      />
    </div>
  );
};
